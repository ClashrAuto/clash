...
func SetTProxyIPTables(ifname string, bypass []string, tport uint16, dnsredir bool, dport uint16) error {
	...
	// set clashauto divert
	execCmd("iptables -t mangle -N clashauto_divert")
	execCmd("iptables -t mangle -F clashauto_divert")
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_divert -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd("iptables -t mangle -A clashauto_divert -j ACCEPT")

	// set pre routing
	execCmd("iptables -t mangle -N clashauto_prerouting")
	execCmd("iptables -t mangle -F clashauto_prerouting")
	execCmd("iptables -t mangle -A clashauto_prerouting -s 172.17.0.0/16 -j RETURN")
	if DnsRedirect {
		execCmd("iptables -t mangle -A clashauto_prerouting -p udp --dport 53 -j ACCEPT")
		execCmd("iptables -t mangle -A clashauto_prerouting -p tcp --dport 53 -j ACCEPT")
	}
	execCmd("iptables -t mangle -A clashauto_prerouting -m addrtype --dst-type LOCAL -j RETURN")
	addLocalnetworkToChain("clashauto_prerouting", bypass)
	execCmd("iptables -t mangle -A clashauto_prerouting -p tcp -m socket -j clashauto_divert")
	execCmd("iptables -t mangle -A clashauto_prerouting -p udp -m socket -j clashauto_divert")
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_prerouting -p tcp -j TPROXY --on-port %d --tproxy-mark %s/%s", tProxyPort, PROXY_FWMARK, PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_prerouting -p udp -j TPROXY --on-port %d --tproxy-mark %s/%s", tProxyPort, PROXY_FWMARK, PROXY_FWMARK))
	execCmd("iptables -t mangle -A PREROUTING -j clashauto_prerouting")
	...
	// set output
	execCmd("iptables -t mangle -N clashauto_output")
	execCmd("iptables -t mangle -F clashauto_output")
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_output -m mark --mark %#x -j RETURN", dialer.DefaultRoutingMark.Load()))
	if DnsRedirect {
		execCmd("iptables -t mangle -A clashauto_output -p udp -m multiport --dports 53,123,137 -j ACCEPT")
		execCmd("iptables -t mangle -A clashauto_output -p tcp --dport 53 -j ACCEPT")
	}
	execCmd("iptables -t mangle -A clashauto_output -m addrtype --dst-type LOCAL -j RETURN")
	execCmd("iptables -t mangle -A clashauto_output -m addrtype --dst-type BROADCAST -j RETURN")
	addLocalnetworkToChain("clashauto_output", bypass)
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_output -p tcp -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -A clashauto_output -p udp -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -I OUTPUT -o %s -j clashauto_output", interfaceName))

	// set dns output
	if DnsRedirect {
		execCmd("iptables -t nat -N clashauto_dns_output")
		execCmd("iptables -t nat -F clashauto_dns_output")
		execCmd(fmt.Sprintf("iptables -t nat -A clashauto_dns_output -m mark --mark %#x -j RETURN", dialer.DefaultRoutingMark.Load()))
		execCmd("iptables -t nat -A clashauto_dns_output -s 172.17.0.0/16 -j RETURN")
		execCmd(fmt.Sprintf("iptables -t nat -A clashauto_dns_output -p udp -j REDIRECT --to-ports %d", dnsPort))
		execCmd(fmt.Sprintf("iptables -t nat -A clashauto_dns_output -p tcp -j REDIRECT --to-ports %d", dnsPort))
		execCmd("iptables -t nat -I OUTPUT -p tcp --dport 53 -j clashauto_dns_output")
		execCmd("iptables -t nat -I OUTPUT -p udp --dport 53 -j clashauto_dns_output")
	}
...
func CleanupTProxyIPTables() {
	...
	if _, err := cmd.ExecCmd("iptables -t mangle -L clashauto_divert"); err != nil {
		return
	}
	...
	execCmd("iptables -t mangle -D PREROUTING -j clashauto_prerouting")
	...
	// clean OUTPUT
	execCmd(fmt.Sprintf("iptables -t mangle -D OUTPUT -o %s -j clashauto_output", interfaceName))
	if DnsRedirect {
		execCmd("iptables -t nat -D OUTPUT -p tcp --dport 53 -j clashauto_dns_output")
		execCmd("iptables -t nat -D OUTPUT -p udp --dport 53 -j clashauto_dns_output")
	}

	// clean chain
	execCmd("iptables -t mangle -F clashauto_prerouting")
	execCmd("iptables -t mangle -X clashauto_prerouting")
	execCmd("iptables -t mangle -F clashauto_divert")
	execCmd("iptables -t mangle -X clashauto_divert")
	execCmd("iptables -t mangle -F clashauto_output")
	execCmd("iptables -t mangle -X clashauto_output")
	if DnsRedirect {
		execCmd("iptables -t nat -F clashauto_dns_output")
		execCmd("iptables -t nat -X clashauto_dns_output")
	}
	interfaceName = ""
	tProxyPort = 0
	dnsPort = 0
