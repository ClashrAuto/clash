...
func SetTProxyIPTables(ifname string, bypass []string, tport uint16, dnsredir bool, dport uint16) error {
	...
	// set coast divert
	execCmd("iptables -t mangle -N coast_divert")
	execCmd("iptables -t mangle -F coast_divert")
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_divert -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd("iptables -t mangle -A coast_divert -j ACCEPT")

	// set pre routing
	execCmd("iptables -t mangle -N coast_prerouting")
	execCmd("iptables -t mangle -F coast_prerouting")
	execCmd("iptables -t mangle -A coast_prerouting -s 172.17.0.0/16 -j RETURN")
	if DnsRedirect {
		execCmd("iptables -t mangle -A coast_prerouting -p udp --dport 53 -j ACCEPT")
		execCmd("iptables -t mangle -A coast_prerouting -p tcp --dport 53 -j ACCEPT")
	}
	execCmd("iptables -t mangle -A coast_prerouting -m addrtype --dst-type LOCAL -j RETURN")
	addLocalnetworkToChain("coast_prerouting", bypass)
	execCmd("iptables -t mangle -A coast_prerouting -p tcp -m socket -j coast_divert")
	execCmd("iptables -t mangle -A coast_prerouting -p udp -m socket -j coast_divert")
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_prerouting -p tcp -j TPROXY --on-port %d --tproxy-mark %s/%s", tProxyPort, PROXY_FWMARK, PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_prerouting -p udp -j TPROXY --on-port %d --tproxy-mark %s/%s", tProxyPort, PROXY_FWMARK, PROXY_FWMARK))
	execCmd("iptables -t mangle -A PREROUTING -j coast_prerouting")
	...
	// set output
	execCmd("iptables -t mangle -N coast_output")
	execCmd("iptables -t mangle -F coast_output")
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_output -m mark --mark %#x -j RETURN", dialer.DefaultRoutingMark.Load()))
	if DnsRedirect {
		execCmd("iptables -t mangle -A coast_output -p udp -m multiport --dports 53,123,137 -j ACCEPT")
		execCmd("iptables -t mangle -A coast_output -p tcp --dport 53 -j ACCEPT")
	}
	execCmd("iptables -t mangle -A coast_output -m addrtype --dst-type LOCAL -j RETURN")
	execCmd("iptables -t mangle -A coast_output -m addrtype --dst-type BROADCAST -j RETURN")
	addLocalnetworkToChain("coast_output", bypass)
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_output -p tcp -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -A coast_output -p udp -j MARK --set-mark %s", PROXY_FWMARK))
	execCmd(fmt.Sprintf("iptables -t mangle -I OUTPUT -o %s -j coast_output", interfaceName))

	// set dns output
	if DnsRedirect {
		execCmd("iptables -t nat -N coast_dns_output")
		execCmd("iptables -t nat -F coast_dns_output")
		execCmd(fmt.Sprintf("iptables -t nat -A coast_dns_output -m mark --mark %#x -j RETURN", dialer.DefaultRoutingMark.Load()))
		execCmd("iptables -t nat -A coast_dns_output -s 172.17.0.0/16 -j RETURN")
		execCmd(fmt.Sprintf("iptables -t nat -A coast_dns_output -p udp -j REDIRECT --to-ports %d", dnsPort))
		execCmd(fmt.Sprintf("iptables -t nat -A coast_dns_output -p tcp -j REDIRECT --to-ports %d", dnsPort))
		execCmd("iptables -t nat -I OUTPUT -p tcp --dport 53 -j coast_dns_output")
		execCmd("iptables -t nat -I OUTPUT -p udp --dport 53 -j coast_dns_output")
	}
...
func CleanupTProxyIPTables() {
	...
	if _, err := cmd.ExecCmd("iptables -t mangle -L coast_divert"); err != nil {
		return
	}
	...
	execCmd("iptables -t mangle -D PREROUTING -j coast_prerouting")
	...
	// clean OUTPUT
	execCmd(fmt.Sprintf("iptables -t mangle -D OUTPUT -o %s -j coast_output", interfaceName))
	if DnsRedirect {
		execCmd("iptables -t nat -D OUTPUT -p tcp --dport 53 -j coast_dns_output")
		execCmd("iptables -t nat -D OUTPUT -p udp --dport 53 -j coast_dns_output")
	}

	// clean chain
	execCmd("iptables -t mangle -F coast_prerouting")
	execCmd("iptables -t mangle -X coast_prerouting")
	execCmd("iptables -t mangle -F coast_divert")
	execCmd("iptables -t mangle -X coast_divert")
	execCmd("iptables -t mangle -F coast_output")
	execCmd("iptables -t mangle -X coast_output")
	if DnsRedirect {
		execCmd("iptables -t nat -F coast_dns_output")
		execCmd("iptables -t nat -X coast_dns_output")
	}
	interfaceName = ""
	tProxyPort = 0
	dnsPort = 0
